...
def contact():
    mess._show(title='Contact us', message="Please contact us on : 'gamerclutch28@gmail.com' ")
...
def resize_bg(event):
    global bg_photo  # Keep a reference to avoid garbage collection

    # Resize the image to match the new window size
    new_width = event.width
    new_height = event.height
    original_bg_image=Image.open("bgimg.jpg")
...
frame1.place(relx=0.11, rely=0.23, relwidth=0.34, relheight=0.70)#box height

frame2 = tk.Frame(window, bg="#c79cff")
frame2.place(relx=0.51, rely=0.23, relwidth=0.34, relheight=0.70)#box height

message4 = tk.Label(window, text="Rohith Institute of Technology" ,fg="white",bg="#2d420a" ,width=55 ,height=1,font=('comic', 29, ' bold '))
message4.place(x=105, y=10)
...
datef.place(relx=0.30, rely=0.17)#3rd row

clock = tk.Label(window, fg="#ff61e5", bg="green", width=20, font=('comic', 15, ' bold '))
clock.place(relx=0.50, rely=0.17)#third row
tick()
...
menubar.add_cascade(label=" " * 476) 

menubar.add_cascade(label="Exit ❌",font=("Arial", 12, "bold"), command=window.destroy)
...
def send_email():
    ...
    full_email = recipient_email + "@" + selected_domain

    from_email = "ilarohith@gmail.com"  # Your email
    
    # Use an App Password instead of your regular password
    app_password = "zonp lybb gjwu xuqc"  # Replace with your App Password
    
    # Check if the attendance file exists
    filename = "Attendance\Attendance_" + date + ".csv"
    if not os.path.exists(filename):
        mess._show(title='Error', message=f'No attendance file found for {date}.')
        return
    ...
    try:
        ...
        text = msg.as_string()
        server.sendmail(from_email, full_email, text)
        server.quit()
        mess._show(title='Success', message=f'Attendance report sent successfully to {full_email}.')
...
delete_registration_button.place(x=20, y=475)
...
delete_attendance_button.place(x=180, y=475)
...
delete_images_button.place(x=340, y=475)
